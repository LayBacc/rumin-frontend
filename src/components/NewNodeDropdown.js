...
export const NewNodeDropdown = (props) => {
  const ref = useRef()
  ...
  const handleActivityResultClick = (e, activity) => {
  	props.addNewNode(activity) 
    props.closeDropdown()
  }

  const handleSpaceResultClick = (e, space) => {
  	props.addNewNode(space)
    props.closeDropdown()
  }
  ...
  const buildResults = () => {
    ...
    return(
      <div
        className="section"
      >
        <div className="section-header padding-small">
          Matching content
        </div>
  ...
  return(
    ...
    <div
      ...
      // }}
    >
      <div className="search-node-input">
        <input 
          ref={inputRef}
          autoFocus
          ...
          style={{paddingLeft: '0.5em', paddingRight: '0.5em', maxWidth: '90%'}}
        />
      </div>
      { buildResults() }
